...
def main(opt):
    ...
    for k, m in model.named_modules():
        ...
        if isinstance(
            m, yolov7_dg.models.common.Conv
        ):  # assign export-friendly activations
            if isinstance(m.act, nn.Hardswish):
                m.act = Hardswish()
            elif isinstance(m.act, nn.SiLU):
                m.act = SiLU()
        # elif isinstance(m, yolov7_dg.models.yolo.Detect):
