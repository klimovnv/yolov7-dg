...
import yolov7_dg.test  # import test.py to get mAP after each epoch
from yolov7_dg.models.experimental import attempt_load
from yolov7_dg.models.yolo import Model
from yolov7_dg.utils.autoanchor import check_anchors
from yolov7_dg.utils.datasets import create_dataloader
...
from yolov7_dg.utils.google_utils import attempt_download
from yolov7_dg.utils.loss import ComputeLoss, ComputeLossOTA
from yolov7_dg.utils.plots import plot_images, plot_labels, plot_results, plot_evolution
from yolov7_dg.utils.torch_utils import (
    ModelEMA,
    select_device,
    intersect_dicts,
...
from yolov7_dg.utils.wandb_logging.wandb_utils import WandbLogger, check_wandb_resume
    ...
    for epoch in range(
        ...
        if rank in [-1, 0]:
            ...
            if not opt.notest or final_epoch:  # Calculate mAP
                ...
                results, maps, times = yolov7_dg.test.test(
                    data_dict,
    ...
    if rank in [-1, 0]:
        ...
        if opt.data.endswith("coco.yaml") and nc == 80:  # if COCO
            for m in (last, best) if best.exists() else (last):  # speed, mAP tests
                results, _, _ = yolov7_dg.test.test(
                    opt.data,
                    batch_size=batch_size * 2,
                    imgsz=imgsz_test,
                    conf_thres=0.001,
...
def main(opt):
    ...
    if opt.resume and not wandb_run:  # resume an interrupted run
        ...
        opt.name = "evolve" if opt.evolve else opt.name + "/train"
